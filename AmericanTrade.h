...
public:
	...
	AmericanOption(const Date& tradeDate, double _notional, double _strike, const Date& _expiry, OptionType _optType)
		: Option(tradeDate, _optType), strike(_strike), notional(_notional), expiryDate(_expiry)
	{
	}
...
{
public:
	AmerCallSpread(const Date& tradeDate, double _notional, double _k1, double _k2, const Date& _expiry)
		:Option(tradeDate, OptionType::Call), strike1(_k1), strike2(_k2), notional(_notional), expiryDate(_expiry)
	{
		assert(_k1 < _k2);
	}
	virtual double Payoff(double S) const
